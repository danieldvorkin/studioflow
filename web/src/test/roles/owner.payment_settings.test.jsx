...
vi.mock("@apollo/client", () => ({
  ...
  useQuery: (query) => {
    ...
    if (queryStr.includes("currentUser")) {
      return {
        data: { currentUser: ownerCurrentUser },
        loading: false,
        refetch: vi.fn(),
      };
    }
    ...
    if (
      queryStr.includes("studioSettings") ||
      queryStr.includes("paymentSettings")
    ) {
      return {
        data: { paymentSettings: queryState.paymentSettings },
...
beforeEach(async () => {
  ...
  if (!OwnerPage) {
    OwnerPage = (await import("../../pages/owner/Owner.jsx")).default;
  }
});
...
describe("Owner – payment settings (Stripe)", () => {
  ...
  it('shows "Save Stripe settings" submit button after unlocking', async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /Save Stripe settings/i }),
    ).toBeInTheDocument();
  });
