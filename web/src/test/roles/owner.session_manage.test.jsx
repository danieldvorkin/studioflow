...
describe('SessionManage — owner/staff session detail panel', () => {
  ...
  it('shows Send reminder button for unpaid booked bookings and calls mutation', async () => {
    ...
    const mocks = [
      sessionBookingsMock([booking]),
      {
        request: { query: SEND_BOOKING_PAYMENT_REMINDER, variables: { bookingId: 'b-1' } },
        result: {
          data: {
  ...
  it('filters roster by status', async () => {
    ...
    const cancelled = makeBooking({
      ...
      status: 'cancelled',
      slug: 'def456',
      client: { __typename: 'Client', id: 'c-2', name: 'John Smith', email: 'john@example.com' },
    })
    ...
    const cancelledFilterBtns = screen.getAllByRole('button', { name: /Cancelled/ })
    // The filter tab contains the count "(1)", so pick the one that matches exactly
    const cancelledFilterBtn = cancelledFilterBtns.find((b) => b.textContent.includes('('))
