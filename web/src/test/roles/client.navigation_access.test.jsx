...
describe("Client role navigation + access guards", () => {
  it("does not show studio-management links (Clients/Classes/Owner) in sidebar", async () => {
    ...
    const myBookingsMock = {
      request: {
        query: MY_BOOKINGS,
        variables: { studioLocationId: null, studioId: null },
      },
    ...
    await screen.findByRole("heading", {
      name: /my bookings|your bookings|bookings/i,
    });
    ...
    expect(
      screen.queryByRole("link", { name: "Instructor payouts" }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("shows restricted message when visiting /clients as a client", async () => {
    ...
    expect(
      screen.getByText(
        /only owners, staff, and moderators can view the client list/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("shows owner access only when visiting /owner as a client", async () => {
    ...
    expect(
      (await screen.findAllByRole("heading", { name: /owner access only/i }))
        .length,
    ).toBeGreaterThan(0);
  });
});
