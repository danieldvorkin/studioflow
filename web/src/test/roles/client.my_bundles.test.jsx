...
// ─── Page import ──────────────────────────────────────────────────────────────
import MyBundlesPage from "../../pages/client/MyBundles.jsx";
import {
...
describe("Client – My Bundles page", () => {
  it("renders the Bundles heading", async () => {
    renderBundles([
      paymentSettingsUnconfiguredMock,
      myClientMock,
      emptyPurchasesMock,
      emptyShopMock,
    ]);
    expect(
      await screen.findByRole("heading", { name: /Bundles/i }),
    ).toBeInTheDocument();
  });
  ...
  it('shows "No active bundles yet" when client has no purchases', async () => {
    ...
    expect(
      await screen.findByText(/No active bundles yet/i),
    ).toBeInTheDocument();
  });
