...
describe("Client Booking payment methods", () => {
  it("can book with a saved card without hitting the own-profile error", async () => {
    const session = {
      ...
      id: "1",
      startTime: new Date("2027-06-15T10:30:00.000Z").toISOString(),
      endTime: new Date("2027-06-15T11:20:00.000Z").toISOString(),
      room: "Room A",
      capacity: 10,
      seatsAvailable: 8,
      classTemplate: {
    ...
    await waitFor(() => {
      ...
      expect(
        calls.some(
          (c) =>
            c?.type === "error" &&
            /Saved card can only be used/i.test(c?.message || ""),
        ),
      ).toBe(false);
      expect(
        calls.some(
          (c) =>
            c?.type === "success" &&
            /Booking confirmed/i.test(c?.message || ""),
        ),
      ).toBe(true);
    });
  });
  ...
  it("can book with bundle credits when the session is bundle-enabled and credits exist", async () => {
    ...
    const mocks = [
      ...
      {
        request: {
          query: BUNDLE_PRODUCTS_FOR_CLASS_SESSION,
          variables: { classSessionId: "1" },
        },
        result: {
          data: {
      ...
      },
      {
        request: {
          ...
          variables: {
            clientId: null,
            classSessionId: "1",
            bundlePurchaseId: "bundle-purchase-1",
          },
        },
        result: {
          data: {
