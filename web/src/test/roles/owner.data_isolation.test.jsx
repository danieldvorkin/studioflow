...
import { studioSettingsMock, currentUserMock, ownerDashboardDataMocks } from '../helpers/apolloMocks'
...
const owner = ownerUser({ id: 'owner-1', studioId: 'studio-1', email: 'owner@studio.com' })
...
describe('Owner data isolation', () => {
  it('does NOT show the Godmode platform picker to a regular owner', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Godmode' })).not.toBeInTheDocument()
  })
  ...
  it('shows the studio-scoped owner workspace heading', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Owner workspace' })).toBeInTheDocument()
  })
  ...
  it('People section shows only staff and instructors, not other owners', async () => {
    ...
    const staffMember = {
      id: 'u-staff-1', email: 'alice@studio.com', name: 'Alice Staff',
      role: 1, roleName: 'staff', active: true, availableForSessions: true,
      godmode: false, studioId: 'studio-1', __typename: 'User',
    }
    const instructorMember = {
      id: 'u-inst-1', email: 'bob@studio.com', name: 'Bob Instructor',
      role: 2, roleName: 'instructor', active: true, availableForSessions: true,
      godmode: false, studioId: 'studio-1', __typename: 'User',
    }
