...
describe('Client – My Memberships page', () => {
  it('renders the My Memberships heading', async () => {
    renderMemberships([
      paymentUnconfiguredMock, myClientMock, myPaymentMethodsMock,
      emptyMembershipsMock, noPlansAvailableMock,
    ])
    // The page-level heading (h1)
    expect(await screen.findByRole('heading', { level: 1, name: /Memberships/i })).toBeInTheDocument()
  })

  it('shows available membership plans', async () => {
    renderMemberships([
      paymentUnconfiguredMock, myClientMock, myPaymentMethodsMock,
      emptyMembershipsMock, plansAvailableMock,
    ])
    expect(await screen.findByText(/Essential Membership/i)).toBeInTheDocument()
  })
  ...
  it('shows "No membership plans are currently available" when no plans exist', async () => {
    ...
    expect(await screen.findByText(/No membership plans are currently available/i)).toBeInTheDocument()
  })
  ...
  it('shows the active membership section when client has an active plan', async () => {
    renderMemberships([
      paymentUnconfiguredMock, myClientMock, myPaymentMethodsMock,
      activeMembershipsMock, noPlansAvailableMock,
    ])
