...
import { CURRENT_USER, BOOKINGS, PAYMENT_PUBLIC_SETTINGS } from '../../apollo/queries.js'
...
describe('Instructor BookingShow payment reminder', () => {
  ...
  it('shows Send payment reminder and calls mutation', async () => {
    ...
    const mocks = [
      ...
      {
        request: { query: SEND_BOOKING_PAYMENT_REMINDER, variables: { bookingId } },
        result: {
          data: {
    ...
    const btn = await screen.findByRole('button', { name: /send payment reminder/i })
    fireEvent.click(btn)
