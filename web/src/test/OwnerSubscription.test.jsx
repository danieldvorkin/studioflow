...
vi.mock('@apollo/client', () => ({
  ...
  useQuery: (() => {
    const impl = () => ({ data: queryState.data, loading: queryState.loading, refetch: vi.fn() })
    return impl
  })(),
  useMutation: (() => {
...
describe('OwnerSubscription page', () => {
  it('renders the page heading', () => {
    ...
    expect(screen.getByText(/StudioFlow plan and billing/i)).toBeInTheDocument()
  })
  ...
  it('shows subdomain and billing date in banner', () => {
    queryState.data = { myStudioSubscription: basicActiveSub() }
    renderSub()
    expect(screen.getByText('awesome-studio.studioflow.app')).toBeInTheDocument()
  ...
  it('shows "Current" badge on the active tier card', () => {
    queryState.data = { myStudioSubscription: basicActiveSub({ tier: 'basic' }) }
    renderSub()
    expect(screen.getByText('Current')).toBeInTheDocument()
  })

  it('shows "Best value" badge on premium card when premium is not the active tier', () => {
    queryState.data = { myStudioSubscription: basicActiveSub({ tier: 'basic' }) }
    renderSub()
    expect(screen.getByText('Best value')).toBeInTheDocument()
  })

  it('does NOT show "Best value" badge when premium is the current active plan', () => {
    queryState.data = { myStudioSubscription: basicActiveSub({ tier: 'premium' }) }
    renderSub()
  ...
  it('renders subscribe buttons when there is no subscription', () => {
    queryState.data = { myStudioSubscription: null }
    renderSub()
    expect(screen.getByRole('button', { name: /Subscribe to Basic/i })).toBeInTheDocument()
  ...
  it('renders subscribe button only for non-current tier when a plan is active', () => {
    queryState.data = { myStudioSubscription: basicActiveSub({ tier: 'basic' }) }
    renderSub()
    // Basic is current — no button, shows contact text instead
    expect(screen.queryByRole('button', { name: /Subscribe to Basic/i })).not.toBeInTheDocument()
    // Premium is not current — shows button
    expect(screen.getByRole('button', { name: /Subscribe to Premium/i })).toBeInTheDocument()
  })
  ...
  it('calls the checkout mutation with the chosen tier on button click', async () => {
    ...
    await waitFor(() => {
      expect(mutationState.fn).toHaveBeenCalledWith({ variables: { tier: 'basic' } })
    })

    await waitFor(() => {
      expect(window.location.href).toBe('https://checkout.stripe.com/pay/test_sess')
    })
  ...
  it('shows past_due alert text when subscription is past due', () => {
    queryState.data = {
      myStudioSubscription: basicActiveSub({ status: 'past_due', active: false }),
    }
    renderSub()
    expect(screen.getByText(/subscription payment is past due/i)).toBeInTheDocument()
    // Multiple billing@ links exist (alert + footer); confirm at least two
    expect(screen.getAllByText(/billing@studioflow.app/i).length).toBeGreaterThanOrEqual(1)
  })

  it('shows cancelled alert text when subscription is cancelled', () => {
    queryState.data = {
      myStudioSubscription: basicActiveSub({ status: 'cancelled', active: false }),
    }
    renderSub()
  ...
  it('shows suspended alert text when subscription is suspended', () => {
    queryState.data = {
      myStudioSubscription: basicActiveSub({ status: 'suspended', active: false }),
    }
    renderSub()
  ...
  it('redirects to /dashboard when user is null', () => {
    ...
    queryState.data = { myStudioSubscription: null }
    renderSub()
    expect(screen.queryByText('No active subscription yet')).not.toBeInTheDocument()
  })
  ...
  it('renders Stripe billing disclaimer text', () => {
    queryState.data = { myStudioSubscription: null }
    renderSub()
    expect(screen.getByText(/Payments are processed securely by Stripe/i)).toBeInTheDocument()
  })
})
