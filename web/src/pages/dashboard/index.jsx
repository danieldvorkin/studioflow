...
export default function Dashboard() {
  ...
  const instructorsByLocation = (() => {
    ...
    for (const t of templates) {
      ...
      const instEntry = locEntry.instructors.get(inst.id) || {
        id: inst.id,
        name: inst.name || inst.email || "Instructor",
        classes: new Set(),
      };
    ...
    return Array.from(byLoc.values())
      .map((loc) => {
        const instructors = Array.from(loc.instructors.values())
          .map((i) => ({
            ...i,
            classes: Array.from(i.classes.values()).slice(0, 3),
          }))
  ...
  const upcomingBookings = [...activeBookings]
    ...
    .sort(
      (a, b) =>
        new Date(a.classSession.startTime) - new Date(b.classSession.startTime),
    )
    .slice(0, 6);
  ...
  const analytics = (() => {
    ...
    const allLast7 = activeBookings.filter((b) =>
      withinDays(b.createdAt, 7, now),
    );
    ...
    const upcoming7 = sessions.filter((s) =>
      futureWithinDays(s.startTime, 7, now),
    );
    ...
    for (const b of paidLast30) {
      ...
      if (!t?.id) continue;
      const prev = templateMap.get(t.id) || {
        title: t.title || "Class",
        grossCents: 0,
        count: 0,
      };
      ...
      templateMap.set(t.id, {
        ...prev,
        grossCents: prev.grossCents + (Number(cents) || 0),
        count: prev.count + 1,
      });
    }
  ...
  const instructorAnalytics = (() => {
    ...
    const upcoming7 = scopedSessions.filter((s) =>
      futureWithinDays(s.startTime, 7, now),
    );
    const fillRates = upcoming7
      .map((s) => {
              ...
              <>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <StatCard
                    ...
                    onClick={
                      ownerLike ? () => setOpenSheet("revenue") : undefined
                    }
                  />
                  <StatCard
                    label="Bookings (7d)"
                    ...
                    onClick={
                      ownerLike ? () => setOpenSheet("funnel") : undefined
                    }
                  />
                  <StatCard
                    label="Fill rate (7d)"
                    value={
                      analytics.avgFillRate == null
                        ? "—"
                        : `${Math.round(analytics.avgFillRate * 100)}%`
                    }
                    helper={`${analytics.upcoming7Count} sessions upcoming`}
                    onClick={
                      ownerLike ? () => setOpenSheet("sessions") : undefined
                    }
                  />
                </div>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-3">
                      ...
                      <StatCard
                        ...
                        value={
                          ownerAnalytics.funnel30.cancelRate == null
                            ? "—"
                            : `${Math.round(ownerAnalytics.funnel30.cancelRate * 100)}%`
                        }
                        ...
                        >
                          <div className="grid grid-cols-1 gap-3">
                            ...
                            <StatRow
                              ...
                              value={
                                ownerAnalytics.clientCohorts?.repeatRate90 ==
                                null
                                  ? "—"
                            ...
                            <StatRow
                              ...
                              value={
                                ownerAnalytics.clientCohorts
                                  ?.avgPaidBookingsPerActive30 == null
                                  ? "—"
                            ...
                            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                              ...
                              <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                                <div className="rounded-lg border border-slate-800 bg-slate-950/20 px-2.5 py-2">
                                  ...
                                  <div className="text-[11px] text-slate-400">
                                    {ownerAnalytics.funnel7.paidCount}/
                                    {ownerAnalytics.funnel7.created} paid
                                  </div>
                                </div>
                                <div className="rounded-lg border border-slate-800 bg-slate-950/20 px-2.5 py-2">
                                  <div className="text-[11px] text-slate-500">
                                  ...
                                  <div className="text-[11px] text-slate-400">
                                    {ownerAnalytics.funnel30.paidCount}/
                                    {ownerAnalytics.funnel30.created} paid
                                  </div>
                                </div>
                              </div>
                            </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                      <div>
                        ...
                        {ownerAnalytics.topInstructorsLast30.length === 0 ? (
                          ...
                          <ul className="mt-2 flex flex-col gap-2">
                            {ownerAnalytics.topInstructorsLast30
                              .slice(0, 8)
                              .map((i) => (
                                <li key={i.id}>
                                  <StatRow
                        ...
                        <div className="mt-2 grid grid-cols-1 gap-3">
                          <div>
                            ...
                            {ownerAnalytics.topClassesLast30.length === 0 ? (
                              ...
                              <ul className="mt-2 flex flex-col gap-2">
                                {ownerAnalytics.topClassesLast30
                                  .slice(0, 5)
                                  .map((t) => (
                                    <li key={t.id}>
                                      <StatRow
                          ...
                          <div>
                            ...
                            {ownerAnalytics.bottomClassesLast30.length === 0 ? (
                              ...
                              <ul className="mt-2 flex flex-col gap-2">
                                {ownerAnalytics.bottomClassesLast30
                                  .slice(0, 5)
                                  .map((t) => (
                                    <li key={t.id}>
                                      <StatRow
              ...
              <>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <StatCard
                    label="Revenue (30d)"
                    value={formatCents(
                      instructorAnalytics?.grossLast30 || 0,
                      currency,
                    )}
                  ...
                  <StatCard
                    label="Fill rate (7d)"
                    value={
                      instructorAnalytics?.avgFillRate == null
                        ? "—"
                        : `${Math.round(instructorAnalytics.avgFillRate * 100)}%`
                    }
            ...
            isClient && (
              <>
                    ...
                    <StatCard
                      ...
                      value={
                        upcomingAllStudiosLoading
                          ? "…"
                          : (upcomingAllStudiosData?.upcomingBookableClassSessionsCount ??
                            0)
                      }
                  ...
                  <div className="text-sm text-slate-300">
                    {upcomingBookings.length ? (
                      <span>
                        ...
                        <span className="font-semibold text-slate-50">
                          {new Date(
                            upcomingBookings[0].classSession.startTime,
                          ).toLocaleString()}
                        </span>
                      </span>
                    ) : (
                ...
                <div className="mt-4">
                  ...
                  {!selectedStudioId ? (
                    <p className="mt-2 rounded-lg border border-dashed border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-400">
                      Select a studio from the studio picker to browse and book
                      sessions.
                    </p>
                            ...
                            <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-400">
                              ...
                              {typeof s.classTemplate?.priceCents ===
                                ...
                                <span>
                                  ·{" "}
                                  {formatCents(
                                    s.classTemplate.priceCents,
                                    s.classTemplate.currency || currency,
                                  )}
                                </span>
                              )}
                              {s._totalSessions > 1 && (
            ...
            {isClient && (
              <div className="mt-4">
                ...
                {templatesLoading ? (
                  ...
                  (() => {
                    const loc = instructorsByLocation.find(
                    ...
                    if (instructors.length === 0) {
                      return (
                        <p className="mt-2 rounded-lg border border-dashed border-slate-700 bg-slate-900/60 p-3 text-sm text-slate-400">
                          No instructors are listed for {selectedLocation.name}{" "}
                          yet.
                        </p>
                      );
                    }
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <StatCard
                      ...
                      value={formatCents(
                        ownerAnalytics.paidLast30GrossCents,
                        currency,
                      )}
                    ...
                    <StatCard
                      ...
                      value={formatCents(
                        ownerAnalytics.paidLast90GrossCents,
                        currency,
                      )}
