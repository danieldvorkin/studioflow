...
function BookingForm({ session, studioIdForBooking, stripeConfigured }) {
  ...
  // Use user-scoped payment methods so cards saved from the Profile page always
  // appear here, regardless of which studio's client record they were saved against.
  const { data: myPaymentMethodsData } = useQuery(MY_PAYMENT_METHODS, {
    skip: !user || !isClientUser,
    fetchPolicy: "cache-and-network",
  });
  const savedMethods =
    myPaymentMethodsData?.myPaymentMethods ||
    myClient?.clientPaymentMethods ||
    [];
  ...
  const bookingForMyClient = !!(
    ...
    (useExistingClient &&
      existingClientId &&
      myClient?.id &&
      existingClientId === myClient.id)
  );

  const canUseSaved = bookingForMyClient && savedMethods.length > 0;

  const shouldShowBundlePayments =
    isClientUser && session?.bundleEnabled === true;
  ...
  const { data: myBundlePurchasesData, loading: myBundlePurchasesLoading } =
  ...
  const eligibleBundlePurchases = useMemo(() => {
    ...
    return purchases.filter(
      (p) =>
        ...
        eligibleBundleProductIds.has(p?.bundleProduct?.id),
    );
  ...
  useEffect(() => {
    ...
    didAutoSelectBundle,
    eligibleBundlePurchases.length,
    shouldShowBundlePayments,
  ]);
  ...
  // Auto-select "saved" when the user has saved cards and hasn't manually chosen yet.
  useEffect(() => {
    if (canUseSaved && paymentChoice === "new" && savedMethods.length > 0) {
      setPaymentChoice("saved");
    }
  }, [canUseSaved, paymentChoice, savedMethods.length]);
  ...
  const existingBooking = isClientUser
    ? (myBookingsData?.myBookings || []).find(
        (b) => b?.status !== "cancelled" && b?.classSession?.id === session?.id,
      )
    : null;

  const existingBookingForRoute = isClientUser
    ? (myBookingsData?.myBookings || []).find(
        (b) =>
          b?.status !== "cancelled" &&
          b?.classSession?.id?.toString() === id?.toString(),
      )
    : null;
  ...
  const onSubmit = async (form) => {
    try {
      ...
      if (!clientIdToUse) {
        if (!isClientUser) {
          ...
          if (clientErrors.length || !client) {
            throw new Error(
              clientErrors.join(", ") || "Could not create client",
            );
          }
      ...
      if (isClientUser && paymentSource === "bundle") {
        if (!selectedBundlePurchaseId)
          throw new Error("Please choose a bundle credit");
      ...
      if (paymentChoice === "saved") {
        if (!canUseSaved) {
          throw new Error(
            "Saved card can only be used when booking for your own profile",
          );
        }
  ...
  if (isClientUser && myBookingsLoading) {
    return (
      <div className="p-6 text-sm text-slate-300">Checking booking status…</div>
    );
  }
    ...
    <div className="mx-auto max-w-5xl space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Confirm a client into this class.
        </p>
      </div>
                ...
                {paymentSource === "bundle" && (
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedBundlePurchaseId(e.target.value)
                      }
                    ...
                    >
                      {!canPayWithBundle && (
                        <option value="">No eligible bundle credits</option>
                      )}
                      {eligibleBundlePurchases.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.bundleProduct?.title || "Bundle"} —{" "}
                          {p.creditsRemaining} credits remaining
                        </option>
                      ))}
                    </select>
                    {!canPayWithBundle && (
              ...
              <>
                <div className="rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2 text-[11px] text-slate-300">
                  ...
                  <div className="mt-0.5 text-slate-400">
                    Card details are sent directly to Stripe for processing. We
                    don’t store your full card number.
                  </div>
                </div>
                ...
                <>
                  ...
                  {import.meta.env.DEV && (
                    <p className="mt-1 text-[11px] text-slate-500">
                      Test mode: use 4242 4242 4242 4242 with any future expiry
                      and CVC.
                    </p>
                  )}
                </>
          ...
          <div className="mt-3 space-y-2 text-xs text-slate-300">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-100">
                {session.classTemplate?.title || "Session"}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Date &amp; time</span>
...
export default function Booking() {
  ...
  const redirectTarget = existingBookingForRoute?.id
    ? `/bookings/${existingBookingForRoute.id}`
    : null;
  const shouldRedirectNow = !!(
    redirectTarget && location?.pathname !== redirectTarget
  );
  ...
  const stripeConfigured =
    paymentPublicSettingsData?.paymentPublicSettings?.configured === true;
  ...
  return (
    <Elements stripe={stripePromise}>
      <BookingForm
        session={session}
        studioIdForBooking={studioIdForBooking}
