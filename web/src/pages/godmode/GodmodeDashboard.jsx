...
function StatCard({ label, value, sub, accent }) {
  ...
  return (
    <div
      className={`flex flex-col gap-1.5 rounded-2xl border p-5 shadow-sm shadow-black/20 ${accentMap[accent] || accentMap.slate}`}
    >
      <div
        className={`text-[10px] font-semibold uppercase tracking-[0.25em] ${labelMap[accent] || labelMap.slate}`}
      >
        {label}
      </div>
...
function TierRow({ tier, count, total }) {
  ...
  const colors = {
    premium: {
      ...
      badge: "border-purple-700 bg-purple-950/40 text-purple-300",
    },
    basic: {
      bar: "bg-sky-500",
      label: "text-sky-300",
      badge: "border-sky-700 bg-sky-950/40 text-sky-300",
    },
  };
  const c = colors[tier] || {
    bar: "bg-slate-500",
    label: "text-slate-300",
    badge: "border-slate-600 bg-slate-900 text-slate-400",
  };

  return (
    <div className="flex items-center gap-3">
      <span
        className={`w-20 shrink-0 rounded-full border px-2 py-0.5 text-center text-[10px] font-semibold uppercase tracking-wide ${c.badge}`}
      >
        {tier}
      </span>
...
export default function GodmodeDashboard() {
  ...
  const tierEntries = s?.subscriptionsByTier
    ? Object.entries(s.subscriptionsByTier)
    : [];
    ...
    <div className="flex w-full flex-col gap-8 p-1">
      ...
      <section className="rounded-2xl border border-emerald-800/50 bg-gradient-to-br from-emerald-950/40 to-slate-900/60 p-6 shadow-sm shadow-black/30">
        ...
        <div className="text-5xl font-bold tracking-tight text-slate-50">
          {loading ? (
            <span className="text-slate-500 text-3xl">Loading…</span>
          ) : (
            fmt(s?.totalRevenueCents)
          )}
        </div>
        <div className="mt-1.5 text-xs text-slate-500">
          {loading
            ? ""
            : `${num(s?.totalPaymentsCount)} successful payment${s?.totalPaymentsCount === 1 ? "" : "s"}`}
        </div>
      </section>

      {/* Studio + subscription grid */}
      <div>
        <div className="mb-3 text-[10px] font-semibold uppercase tracking-[0.3em] text-slate-500">
          Studios
        </div>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard
            ...
            sub={
              loading || !s
                ? ""
                : `${num(s.newStudiosThisMonth)} new this month`
            }
            accent="violet"
          />
          <StatCard
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          ...
          <StatCard
            ...
            sub={
              bookingConfirmRate != null
                ? `${bookingConfirmRate}% confirmation rate`
                : ""
            }
            accent="emerald"
          />
