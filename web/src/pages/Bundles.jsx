...
import { BUNDLE_PRODUCTS, CLASS_TEMPLATES, CURRENT_USER, INSTRUCTORS } from '../apollo/queries'
import { CREATE_BUNDLE_PRODUCT, DELETE_BUNDLE_PRODUCT, UPDATE_BUNDLE_PRODUCT } from '../apollo/mutations'
import { useToast } from '../components/ToastProvider'
import { useAuth } from '../auth/AuthProvider'
import { isGodmode as isGodmodeUser, isOwner, isStaff } from '../auth/permissions'
...
export default function BundlesPage() {
  ...
  if (!canManage) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-800 bg-slate-900/80 p-6 text-sm text-slate-200">
        <h1 className="mb-2 text-lg font-semibold text-slate-50">Owner/staff access only</h1>
        <p className="text-sm text-slate-400">Only staff and the studio owner can manage bundles.</p>
      </div>
    )
  }
  ...
  const submitCreate = async (e) => {
    ...
    if (!createForm.classTemplateId && !createForm.instructorId) {
      addToast({ message: 'Select a class template and/or instructor', type: 'error' })
      return
    }
    ...
    try {
      ...
      if (errors.length || !payload?.bundleProduct) throw new Error(errors.join(', ') || 'Create failed')
    ...
    try {
      const res = await updateBundleProduct({
        variables: {
          ...
          creditsCount: editForm.creditsCount ? Number(editForm.creditsCount) : null,
          priceCents: editForm.priceDollars ? centsFromDollarsInput(editForm.priceDollars) : null,
      ...
      const errors = payload?.errors || []
      if (errors.length || !payload?.bundleProduct) throw new Error(errors.join(', ') || 'Update failed')
  ...
  const handleDeactivate = async (id) => {
    ...
    try {
      ...
      if (!payload?.success) throw new Error((payload?.errors || ['Deactivate failed']).join(', '))
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-400">Create packages of credits that can be redeemed for classes.</p>
        {isGodmode && auth.isImpersonating && auth.impersonator?.email ? (
          <p className="mt-1 text-xs text-slate-500">Impersonating as {auth.impersonator.email}</p>
        ) : null}
      </header>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm shadow-black/20">
        ...
        <form className="grid grid-cols-1 gap-3 md:grid-cols-2" onSubmit={submitCreate}>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">Credits</label>
            <input
              type="number"
              min="1"
              value={createForm.creditsCount}
              onChange={(e) => setCreateForm((v) => ({ ...v, creditsCount: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
              required
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">Price</label>
            <input
              type="number"
              value={createForm.priceDollars}
              onChange={(e) => setCreateForm((v) => ({ ...v, priceDollars: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
            />
            <p className="text-[11px] text-slate-500">Charged when purchasing this bundle.</p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              onChange={(e) => setCreateForm((v) => ({ ...v, currency: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
            >
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">Class template (optional)</label>
            <select
              value={createForm.classTemplateId}
              onChange={(e) => setCreateForm((v) => ({ ...v, classTemplateId: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
            >
              <option value="">—</option>
              {templates.map((t) => (
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">Instructor (optional)</label>
            <select
              value={createForm.instructorId}
              onChange={(e) => setCreateForm((v) => ({ ...v, instructorId: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
            >
              <option value="">—</option>
              {instructors.map((i) => (
          ...
          <div className="md:col-span-2 flex items-center justify-between gap-2">
            <label className="flex items-center gap-2 text-xs font-medium text-slate-200">
              <input
                ...
                onChange={(e) => setCreateForm((v) => ({ ...v, active: e.target.checked }))}
              />
              Active
            </label>
          ...
          <div className="md:col-span-2 text-[11px] text-slate-500">
            Note: A bundle must target at least a class template or an instructor.
          </div>
        </form>
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm shadow-black/20">
        ...
        {!loading && bundleProducts.length === 0 && <p className="text-sm text-slate-500">No bundles yet.</p>}
                      ...
                      <td className="px-3 py-2 text-slate-300">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, creditsCount: e.target.value }))}
                      ...
                      <td className="px-3 py-2 text-slate-300">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, priceDollars: e.target.value }))}
                          ...
                          typeof bp.priceCents === 'number' ? dollarsFromCents(bp.priceCents) : '—'
                        )}
                      </td>
                      ...
                      <td className="px-3 py-2 text-slate-400">
                        {isEditing ? (
                          <select
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, classTemplateId: e.target.value }))}
                      ...
                      <td className="px-3 py-2 text-slate-400">
                        {isEditing ? (
                          <select
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, instructorId: e.target.value }))}
                      ...
                      <td className="px-3 py-2 text-slate-400">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, active: e.target.checked }))}
                          />
                        ) : (
                          bp.active ? 'Yes' : 'No'
                        )}
