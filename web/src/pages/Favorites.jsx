...
import { MY_FAVORITE_CLASS_SESSIONS, STUDIO_LOCATIONS } from '../apollo/queries'
...
function formatDateTime(isoString) {
  if (!isoString) return ''
  return new Date(isoString).toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })
}

export default function Favorites() {
  const { studios, selectedStudioId } = useStudio()
  ...
  const { data, loading, error, refetch } = useQuery(MY_FAVORITE_CLASS_SESSIONS, {
  ...
  const [toggleFavorite, { loading: toggling }] = useMutation(TOGGLE_FAVORITE_CLASS_SESSION)
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-400">Your saved sessions, with studio and location filters.</p>
      </header>
      ...
      <section className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-sm md:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          {!studioIdFilter && (
            <div className="text-[11px] text-slate-500">Choose a studio to filter by location.</div>
          )}
        </div>
      </section>
      ...
      {!loading && favoriteSessions.length === 0 && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 text-sm text-slate-300">
          <div className="font-semibold text-slate-50">No favorites yet</div>
          <div className="mt-1 text-slate-400">Tap the heart on any session to save it here.</div>
        </div>
      )}
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs">
                <button
                  ...
                  onClick={async () => {
                    try {
                      const res = await toggleFavorite({ variables: { classSessionId: s.id } })
                      ...
                      addToast({ message: e.message || 'Failed to update favorite', type: 'error' })
                    }
                  }}
