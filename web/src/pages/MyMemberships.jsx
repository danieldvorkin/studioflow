...
function PerkList({ plan }) {
  ...
  if (plan.reformerClassesPerMonth == null) {
    ...
    perks.push(
      `${plan.reformerClassesPerMonth} Reformer class${plan.reformerClassesPerMonth !== 1 ? "es" : ""} / month`,
    );
  }

  if (plan.matClassesPerMonth != null) {
    perks.push(
      `${plan.matClassesPerMonth} Mat / Barre / Yoga class${plan.matClassesPerMonth !== 1 ? "es" : ""} / month`,
    );
  }

  if (plan.includesPriorityBooking) perks.push("Priority Booking");
  if (plan.includesEarlyBooking) perks.push("Early Booking Access");
  if (plan.privateSessionDiscountPercent > 0)
    perks.push(`${plan.privateSessionDiscountPercent}% Off Private Sessions`);
...
function PurchaseMembershipCard({
  ...
  const onSubscribe = async (e) => {
    ...
    try {
      ...
      if (errors.length || !payload?.clientMembership)
        throw new Error(errors.join(", ") || "Enrollment failed");
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto pt-3 text-xs text-slate-500">
        {plan.minCommitmentMonths}-month commitment ·{" "}
        {plan.autoRenew ? "Auto-renews" : "No auto-renew"}
      </div>
      ...
      {!enrolled && expanded && (
        <form
          ...
          className="mt-2 space-y-3 border-t border-slate-800 pt-3"
        >
          {!isFree && (
            <>
              ...
              {paymentChoice === "new" && !canUseNewCard && (
                <div className="text-xs text-slate-400">
                  New-card checkout is unavailable. Add a saved card in Profile,
                  or try again later.
                </div>
              )}
            </>
  ...
  const content = (
    <div className="flex w-full flex-col gap-6">
      ...
      {(membershipsLoading || myMemberships.length > 0) && (
        <section>
          <h2 className="text-base font-semibold uppercase tracking-widest text-sky-400 mb-3">
            My Memberships
          </h2>
          {membershipsLoading && (
              ...
              >
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${STATUS_COLORS[m.status] || ""}`}
                    >
                      {m.status}
                    </span>
                  </div>
                  ...
                  <div className="mt-2 text-xs text-slate-500">
                    ...
                    {m.membershipPlan?.minCommitmentMonths > 0 && (
                      <>
                        {" "}
                        · {m.membershipPlan.minCommitmentMonths}-month min.
                        commitment
                      </>
                    )}
