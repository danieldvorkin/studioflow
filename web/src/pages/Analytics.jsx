...
import { BOOKINGS, CLASS_SESSIONS, CLASS_TEMPLATES, CURRENT_USER } from '../apollo/queries'
...
export default function Analytics() {
  ...
  const { data: sessionsData, loading: sessionsLoading } = useQuery(CLASS_SESSIONS, {
    variables: { from: null, to: null, studioLocationId: locationId || null, studioId: null },
    skip: !ownerLike,
  })
  ...
  const ownerAnalytics = (() => {
    ...
    const paidLast30GrossCents = paidLast30.reduce((sum, b) => sum + grossFor(b), 0)
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-400">Deeper views of the dashboard summary for owners and staff.</p>
      </header>
          ...
          >
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Paid revenue (30d)</div>
                <div className="mt-1 text-lg font-semibold text-slate-50">
                  {formatCents(ownerAnalytics.paidLast30GrossCents, currency)}
                </div>
                <div className="mt-0.5 text-xs text-slate-400">{ownerAnalytics.paidLast30Count} paid bookings</div>
              </div>
              <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">This week vs last</div>
                <div className="mt-1 text-lg font-semibold text-slate-50">
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Paid revenue (90d)</div>
                <div className="mt-1 text-lg font-semibold text-slate-50">
                  {formatCents(ownerAnalytics.paidLast90GrossCents, currency)}
                </div>
                <div className="mt-0.5 text-xs text-slate-400">{ownerAnalytics.paidLast90Count} paid bookings</div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              <StatRow
                label="Highest week"
                value={ownerAnalytics.highestWeek ? formatCents(ownerAnalytics.highestWeek.grossCents, currency) : '—'}
              ...
              <StatRow
                label="Lowest week"
                value={ownerAnalytics.lowestWeek ? formatCents(ownerAnalytics.lowestWeek.grossCents, currency) : '—'}
                    ...
                    {upcoming7.map((s) => {
                      ...
                      return (
                        <tr key={s.id} className="border-t border-slate-800">
                          ...
                          <td className="px-3 py-2 text-xs text-slate-300">{s.instructor?.name || '—'}</td>
                          <td className="px-3 py-2 text-xs text-slate-300">{cap ?? '—'}</td>
                          <td className="px-3 py-2 text-xs text-slate-300">{seats ?? '—'}</td>
                  ...
                  <tbody>
                    {ownerAnalytics.topInstructorsLast30.map((i) => (
                      <tr key={i.id} className="border-t border-slate-800">
                        ...
                        <td className="px-3 py-2 text-xs text-slate-300">
                          {i.cancelRate == null ? '—' : `${Math.round(i.cancelRate * 100)}%`}
                        </td>
