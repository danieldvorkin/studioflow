...
function Badge({ label, colorClass }) {
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${colorClass}`}>
      {label}
    </span>
  )
}
...
function EditModal({ studio, subscription, onClose, onSaved }) {
  ...
  const [stripeCustomerId, setStripeCustomerId] = useState(subscription?.stripeCustomerId || '')
  const [stripeSubscriptionId, setStripeSubscriptionId] = useState(subscription?.stripeSubscriptionId || '')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-400">Current period end</label>
            <input
              type="date"
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-400">Stripe customer ID (platform)</label>
            <input
              type="text"
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-400">Stripe subscription ID (platform)</label>
            <input
              type="text"
...
export default function GodmodeSubscriptions() {
  ...
  const { data: subsData, loading: subsLoading } = useQuery(STUDIO_SUBSCRIPTIONS, {
    skip: !isGodmode,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-lg font-semibold text-white">Platform Subscriptions</h1>
        <p className="mt-1 text-sm text-slate-400">Manage all studio owners and their StudioFlow subscriptions.</p>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-700 bg-slate-900">
        <table className="min-w-full divide-y divide-slate-800 text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {loading && (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-sm text-slate-500">Loading…</td>
              </tr>
            )}
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-sm text-slate-500">No studios yet.</td>
              </tr>
            )}
            {!loading && rows.map(({ studio, subscription: sub }) => (
              <tr key={studio.id} className="hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-3 font-medium text-white">{studio.name}</td>
                <td className="px-4 py-3 text-slate-400 font-mono text-xs">
                  {studio.slug ? (
                    <span className="rounded bg-slate-800 px-1.5 py-0.5">{studio.slug}</span>
                  ) : (
                ...
                <td className="px-4 py-3">
                  {sub ? (
                    <Badge
                      ...
                      colorClass={TIER_COLORS[sub.tier] || 'bg-slate-800 text-slate-400 border border-slate-600'}
                    />
                  ) : (
                ...
                <td className="px-4 py-3">
                  {sub ? (
                    <Badge
                      ...
                      colorClass={STATUS_COLORS[sub.status] || 'bg-slate-800 text-slate-400 border border-slate-600'}
                    />
                  ) : (
                ...
                <td className="px-4 py-3 text-slate-300 text-xs">
                  {sub ? TIER_PRICES[sub.tier] || `$${sub.priceCad} CAD` : '—'}
                </td>
                ...
                <td className="px-4 py-3 text-slate-500 font-mono text-[10px] space-y-0.5">
                  ...
                  {sub?.stripeSubscriptionId && <div>{sub.stripeSubscriptionId}</div>}
                  {!sub?.stripeCustomerId && !sub?.stripeSubscriptionId && '—'}
                </td>
                <td className="px-4 py-3 text-right">
                  <button
                    type="button"
                    onClick={() => setEditTarget({ studio, subscription: sub })}
