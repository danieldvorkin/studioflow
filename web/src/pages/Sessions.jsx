...
import { CLASS_SESSIONS, CLASS_TEMPLATES, MY_BOOKINGS, MY_FAVORITE_CLASS_SESSIONS } from '../apollo/queries'
import { UPDATE_CLASS_SESSION, DELETE_CLASS_SESSION, TOGGLE_FAVORITE_CLASS_SESSION } from '../apollo/mutations'
...
export default function Sessions() {
  ...
  const { data: templatesData } = useQuery(CLASS_TEMPLATES, {
    variables: isClientUser && selectedStudioId ? { studioId: selectedStudioId } : undefined,
  ...
  if (error) return <p className="text-sm text-rose-400">Error loading sessions</p>

  const sessions = (data?.classSessions || []).filter((s) => s.classTemplate?.id === id)
  const myBookings = (myBookingsData?.myBookings || []).filter((b) => b?.status !== 'cancelled')
  const bookingBySessionId = new Map(myBookings.map((b) => [b?.classSession?.id, b]).filter(([sid]) => !!sid))

  const template = (templatesData?.classTemplates || []).find((t) => t.id === id) || sessions[0]?.classTemplate || null
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4">
      <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            <h1 className="text-2xl font-semibold text-slate-50 truncate">{template?.title || 'Class'}</h1>
            {template?.description ? (
              <p className="mt-2 text-sm text-slate-300">{template.description}</p>
            ) : (
              <p className="mt-2 text-sm text-slate-400">Upcoming scheduled classes for this program.</p>
            )}
            <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-slate-400">
              {template?.durationMinutes ? <span>{template.durationMinutes} min</span> : null}
              {template?.studioLocation?.name ? <span>• {template.studioLocation.name}</span> : null}
              {template?.instructor?.name ? <span>• {template.instructor.name}</span> : null}
              {typeof template?.priceCents === 'number' ? (
                <span>• {(template.priceCents / 100).toFixed(2)} {(template.currency || 'cad').toUpperCase()}</span>
              ) : null}
            </div>
          </div>
      ...
      {sessions.length === 0 ? (
        <p className="text-sm text-slate-500">No sessions have been scheduled yet.</p>
      ) : (
                    ...
                    {canEditSession(user, s) && (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await updateSession({ variables: { id: s.id, room: newRoom || null } })
                    ...
                    {canDelete && (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await deleteSession({ variables: { id: s.id } })
