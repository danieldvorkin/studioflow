...
export default function StudioShowPage() {
  ...
  if (!studio)
    return <div className="text-sm text-slate-400">Studio not found.</div>;
                        ...
                        {s.classTemplate?.priceCents > 0 && (
                          <div className="text-sm font-semibold text-sky-400">
                            {formatPrice(
                              ...
                              s.classTemplate.currency,
                            )}
                          </div>
                        )}
                        <div
                          className={`text-[11px] ${full ? "text-red-400" : "text-slate-400"}`}
                        >
                          {full
                            ? "Full"
                            : `${s.seatsAvailable} spot${s.seatsAvailable === 1 ? "" : "s"} left`}
                        </div>
                        {isSelected && !full && (
                          <Link
                            to={`/booking/${s.id}`}
