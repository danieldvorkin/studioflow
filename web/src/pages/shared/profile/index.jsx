...
export default function Profile() {
  ...
  const [updateClientMembership, { loading: cancellingMembership }] =
    useMutation(UPDATE_CLIENT_MEMBERSHIP);

  const cancelMembership = async (id) => {
    if (!window.confirm("Cancel this membership? This cannot be undone."))
      return;
    try {
      const res = await updateClientMembership({
      ...
      addToast({
        message: err.message || "Could not cancel membership",
        type: "error",
      });
    }
  };

  const [createSetupIntent, { loading: creatingSetupIntent }] =
    useMutation(CREATE_SETUP_INTENT);
  const [saveMyPaymentMethod, { loading: savingCard }] = useMutation(
    SAVE_MY_PAYMENT_METHOD,
  );
  const [removeMyPaymentMethod, { loading: removingCard }] = useMutation(
    REMOVE_MY_PAYMENT_METHOD,
  );
  const [setMyDefaultPaymentMethod, { loading: settingDefaultCard }] =
    useMutation(SET_MY_DEFAULT_PAYMENT_METHOD);
  ...
  const platformPublishableKey =
    platformSettingsData?.platformPaymentSettings?.stripePublishableKey;
  ...
  const _ownerHasActiveSub =
    ownerSub?.status &&
    ["active", "trialing", "past_due"].includes(ownerSub.status);
            ...
            <input
              ...
              placeholder={
                requiresCurrentPassword
                  ...
                  : "Only required when changing email or password"
              }
            />
          </div>
...
function SavedCardEditor({
  ...
  const onMakeDefault = async (paymentMethodId) => {
    try {
      ...
      const errors = payload?.errors || [];
      if (errors.length)
        throw new Error(errors.join(", ") || "Could not set default");
      addToast({ message: "Default card updated", type: "success" });
      onUpdated?.();
    } catch (err) {
      addToast({
        message: err.message || "Could not set default",
        type: "error",
      });
    }
  };
  ...
  const onRemove = async (paymentMethodId) => {
    try {
      ...
      const errors = payload?.errors || [];
      if (errors.length)
        throw new Error(errors.join(", ") || "Could not remove card");
      addToast({ message: "Card removed", type: "success" });
      onUpdated?.();
    } catch (err) {
      addToast({
        message: err.message || "Could not remove card",
        type: "error",
      });
    }
  };
