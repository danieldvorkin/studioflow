...
export default function LocationsPage() {
  ...
  const [createForm, setCreateForm] = useState({ name: '', address: '', city: '', state: '', zip: '' })
  const [editingId, setEditingId] = useState(null)
  const [editForm, setEditForm] = useState({ name: '', address: '', city: '', state: '', zip: '' })
  ...
  if (!isOwner) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-800 bg-slate-900/80 p-6 text-sm text-slate-200">
        ...
        <p className="text-sm text-slate-400">Only the studio owner can manage locations.</p>
      </div>
    )
  }
  ...
  const submitCreate = async (e) => {
    ...
    try {
      ...
      if (errors.length || !payload?.studioLocation) throw new Error(errors.join(', ') || 'Create failed')
  ...
  const submitEdit = async () => {
    try {
      ...
      const errors = payload?.errors || []
      if (errors.length || !payload?.studioLocation) throw new Error(errors.join(', ') || 'Update failed')
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm shadow-black/20">
        ...
        <form className="grid grid-cols-1 gap-3 md:grid-cols-2" onSubmit={submitCreate}>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">City</label>
            <input
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">State / Province</label>
            <input
              value={createForm.state}
              onChange={(e) => setCreateForm((v) => ({ ...v, state: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
              placeholder="ON"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-300">ZIP / Postal</label>
            <input
              value={createForm.zip}
              onChange={(e) => setCreateForm((v) => ({ ...v, zip: e.target.value }))}
              className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
              placeholder="M5V 2T6"
            />
          </div>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm shadow-black/20">
        ...
        {loading && <p className="text-sm text-slate-400">Loading locations…</p>}
        {!loading && locations.length === 0 && <p className="text-sm text-slate-500">No locations yet.</p>}
                    ...
                    <tr key={loc.id} className="border-t border-slate-800">
                      ...
                      <td className="px-3 py-2 text-slate-400">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => setEditForm((v) => ({ ...v, state: e.target.value }))}
                            className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                          />
                        ) : (
                          loc.state || '—'
                      ...
                      <td className="px-3 py-2 text-slate-400">
                        {isEditing ? (
                          <input
                            value={editForm.zip}
                            onChange={(e) => setEditForm((v) => ({ ...v, zip: e.target.value }))}
                            className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                          />
                        ) : (
                          loc.zip || '—'
