...
export default function ClientProfile() {
  useDocumentTitle("Client Profile");
  const { id } = useParams();
  ...
  const classesSummary = useMemo(() => {
    ...
    for (const b of bookings) {
      ...
      byTitle.set(title, {
        ...
        count: prev.count + 1,
        lastAt:
          !prevLast || (nextLast && nextLast > prevLast) ? when : prev.lastAt,
      });
    }
  ...
  if (!user) {
    return (
      <p className="text-sm text-slate-400">Sign in to view client profiles.</p>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-800 bg-slate-900/80 p-6 text-sm text-slate-200">
        ...
        <p className="text-sm text-slate-400">
          Only owners, staff, and instructors can view client profiles.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      <header className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-400">
              A quick view of history, notes, and instructor-facing
              characteristics.
            </p>
          </div>
          <Link
      ...
      {loading && <p className="text-sm text-slate-400">Loading client…</p>}
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
              ...
              <div className="mt-3 space-y-2">
                {scoresDraft.length === 0 && (
                  <p className="text-sm text-slate-400">
                    No characteristics yet. Add a few scoring rows to help
                    instructors quickly understand this client.
                  </p>
                )}
                ...
                {scoresDraft.map((row, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-1 gap-2 rounded-xl border border-slate-800 bg-slate-950/40 p-3 md:grid-cols-[minmax(0,1fr)_7rem_minmax(0,1fr)_auto]"
                  >
                    <input
                      value={row.label}
                      onChange={(e) =>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (errors.length || !payload?.client)
                        throw new Error(
                          errors.join(", ") || "Could not save scores",
                        );
                      addToast({
                      ...
                      addToast({
                        message: e.message || "Could not save characteristics",
                        type: "error",
                      });
                    } finally {
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        setAddingNote(true);
                        const res = await createClientNote({
                          variables: {
                            clientId: client.id,
                        ...
                        const errors = payload?.errors || [];
                        if (errors.length || !payload?.note)
                          throw new Error(
                            errors.join(", ") || "Could not add note",
                          );
                        setNewNoteBody("");
                        ...
                        addToast({
                          message: e.message || "Could not add note",
                          type: "error",
                        });
                      } finally {
                        setAddingNote(false);
                      ...
                      <tbody>
                        {bookings.map((b) => (
                          <tr key={b.id} className="border-t border-slate-800">
                            <td className="px-3 py-2 text-slate-300">
                              {formatDateTime(
                                b?.classSession?.startTime || b.createdAt,
                              )}
                            </td>
                            ...
                            <td className="px-3 py-2">
                              <span
                                className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${b.paid ? "bg-emerald-500/10 text-emerald-200" : "bg-slate-800 text-slate-400"}`}
                              >
                                {b.paid ? "Paid" : "Unpaid"}
                              </span>
                            </td>
