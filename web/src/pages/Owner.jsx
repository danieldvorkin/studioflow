  ...
  if (!isOwner) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-800 bg-slate-900/80 p-6 text-sm text-slate-200">
        <h1 className="mb-2 text-lg font-semibold text-slate-50">Owner access only</h1>
...
function OwnerModules({
      ...
      render: () => (
        <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
          {(() => {
            ...
            const clientsPageEnabled = paymentSettings?.clientsPageEnabled !== false
              ...
              {inviteOpen && (
                <div className="border-b border-slate-800 bg-slate-900/40 px-3 py-3">
                  <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (!email) {
                          addToast({ message: 'Email is required', type: 'error' })
                          return
                        }
                        ...
                        try {
                          ...
                          addToast({ message: 'Invite sent', type: 'success' })
                          setInviteEmail('')
                          setInviteName('')
                          setInviteRole(1)
                          setInviteOpen(false)
                          ...
                          addToast({ message: e.message || 'Could not invite user', type: 'error' })
                        }
                      }}
                  ...
                  <p className="mt-2 text-[11px] text-slate-500">
                    Creates the account in your studio and emails a password setup link.
                  </p>
                </div>
              )}

              <div className="min-h-0 flex-1 overflow-auto">
                {loading && <p className="p-3 text-sm text-slate-400">Loading users…</p>}
                {!loading && filteredUsers.length === 0 && (
                  <p className="p-3 text-sm text-slate-500">No users match your search.</p>
                )}
                      ...
                      {filteredUsers.map((u) => (
                        <tr key={u.id} className="border-t border-slate-800">
                          ...
                          <td className="px-3 py-2 text-slate-300">
                            {u.id === currentUser.id ? (
                              <span className="uppercase">{u.roleName}</span>
                            ) : (
                              <select
                                ...
                                value={u.role}
                                onChange={async (e) => {
                                  const res = await updateUser({ variables: { id: u.id, role: Number(e.target.value) } })
                                  const errors = res.data?.updateUser?.errors || []
                                  if (errors.length) {
                                    addToast({ message: errors.join(', '), type: 'error' })
                                  } else {
                                    addToast({ message: 'Role updated', type: 'success' })
                                    refetch()
                                  }
                                }}
                              >
                          ...
                          <td className="px-3 py-2">
                            <button
                              type="button"
                              onClick={() => handleToggle(u, 'availableForSessions')}
                            ...
                            >
                              {u.availableForSessions ? 'Available' : 'Unavailable'}
                            </button>
                          </td>
            ...
            <div className="flex min-h-0 flex-col rounded-xl border border-slate-800">
              <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900/70 px-3 py-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          addToast({ message: next ? 'Clients page shown' : 'Clients page hidden', type: 'success' })
                        } catch (err) {
                  ...
                  <span className="text-[11px] text-slate-500">{filteredClients.length}</span>
                </div>
              </div>
              <div className="min-h-0 flex-1 overflow-auto">
                {clientsLoading && <p className="p-3 text-xs text-slate-400">Loading clients…</p>}
                {!clientsLoading && filteredClients.length === 0 && (
                  <p className="p-3 text-xs text-slate-500">No clients match your search.</p>
                )}
                    ...
                    <tbody>
                      {filteredClients.map((c) => (
                        <tr key={c.id} className="border-t border-slate-800">
                          ...
                          <td className="px-3 py-2">
                            {c.user?.id ? (
                              <span className="inline-flex items-center rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-300">Yes</span>
                            ) : (
                              <span className="inline-flex items-center rounded-full bg-slate-800 px-2 py-0.5 text-[11px] font-medium text-slate-400">No</span>
                            )}
                          </td>
                          <td className="px-3 py-2 text-right">
                            {c.user?.id && c.user.id !== currentUser.id ? (
                              <button
                                type="button"
                                onClick={() => handleViewAs({ id: c.user.id })}
                                className="inline-flex items-center rounded-full border border-slate-700 px-2 py-0.5 text-[11px] text-slate-100 hover:border-sky-400 hover:text-sky-300"
                              >
                                View as
                              </button>
      ...
      render: () => (
        <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-3 text-xs text-slate-200">
          ...
          <div className="min-h-0 flex-1">
            {bookingsLoading && <p className="text-xs text-slate-400">Loading bookings…</p>}
            {!bookingsLoading && bookings.length === 0 && <p className="text-xs text-slate-500">No bookings yet.</p>}
            {!bookingsLoading && bookings.length > 0 && (
              <ul className="flex h-full flex-col gap-1 overflow-auto">
                {bookings.slice(0, 50).map((b) => (
                  <li key={b.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-800 bg-slate-900/90 px-2 py-1.5">
                    <div className="flex flex-col">
                      <span className="text-[11px] text-slate-50">
                        {b.client.name} → {b.classSession.classTemplate?.title}
                      </span>
                      <span className="text-[10px] text-slate-400">
                        {new Date(b.classSession.startTime).toLocaleString()} · {b.status}
                      </span>
                    </div>
                    <button
                      type="button"
    ...
    },
    {
      ...
      render: () => (
        <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-3 text-xs text-slate-200">
          ...
          <div className="min-h-0 flex-1">
            {paymentsLoading && <p className="text-xs text-slate-400">Loading payments…</p>}
            {!paymentsLoading && payments.length === 0 && <p className="text-xs text-slate-500">No payments recorded yet.</p>}
            {!paymentsLoading && payments.length > 0 && (
              <ul className="flex h-full flex-col gap-1 overflow-auto">
                {payments.slice(0, 80).map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-800 bg-slate-900/90 px-2 py-1.5">
                    <div className="flex flex-col">
                      <span className="text-[11px] text-slate-50">
                        {p.client?.name || p.client?.email || 'Client'} → {p.classSession?.classTemplate?.title || 'Class'}
                      </span>
                      <span className="text-[10px] text-slate-400">
                        {new Date(p.createdAt).toLocaleString()} · ${(p.amountCents / 100).toFixed(2)} {p.currency.toUpperCase()} · {p.status}
                      </span>
                    </div>
                  </li>
                ))}
  ...
  useEffect(() => {
    ...
    const coerceLayout = (raw) => {
      ...
      const rawOrder = Array.isArray(raw.order) ? raw.order.filter((id) => typeof id === 'string') : []
      const order = [...new Set(rawOrder.map((id) => (id === 'users' || id === 'clients') ? 'people' : id))]

      const rawHeights = (raw.heights && typeof raw.heights === 'object') ? raw.heights : {}
      const rawWidths = (raw.widths && typeof raw.widths === 'object') ? raw.widths : {}

      const heights = { ...rawHeights }
      const widths = { ...rawWidths }

      if (heights.people == null && (heights.users != null || heights.clients != null)) {
        heights.people = Math.max(Number(heights.users) || 0, Number(heights.clients) || 0)
      }
      if (widths.people == null && (widths.users != null || widths.clients != null)) {
        widths.people = Math.max(Number(widths.users) || 0, Number(widths.clients) || 0)
      }
    ...
    const initial = fromServer || fromLocal || { version: 1, template: 'default', order: [], heights: {}, widths: {} }
  ...
  }, [layoutInitialized, layoutStorageKey, moduleIdsKey, paymentSettings, persistToLocalStorage])

  const orderedIds = (Array.isArray(layoutOrder) && layoutOrder.length)
    ? layoutOrder
  ...
  }, [customLayoutPayload, layoutInitialized, layoutTemplate, paymentSettings?.id, updatePaymentSettings])

  const resetLayout = async () => {
    const cleared = { version: 1, template: 'default', order: [], heights: {}, widths: {} }
    ...
    try {
      ...
      addToast({ message: e.message || 'Could not reset layout', type: 'error' })
    }
  }
        ...
        <SortableContext items={orderedIds} strategy={rectSortingStrategy}>
          <section className="grid grid-cols-1 gap-3 auto-rows-[10px] grid-flow-dense md:grid-cols-2 xl:grid-cols-3">
            {orderedModules.map((m) => {
              ...
              const effectiveHeight = (typeof rawHeight === 'number') ? rawHeight : (m.defaultHeight || DEFAULT_MODULE_HEIGHT)
              const rawWidth = moduleWidths?.[m.id]
              const effectiveWidth = Math.max(1, Math.min(3, (typeof rawWidth === 'number') ? rawWidth : (m.defaultWidth || 1)))
              ...
              return (
                <SortableOwnerModule
                  ...
                  isDropTarget={activeId && overId === m.id && activeId !== m.id}
                  onResize={(nextHeight) => {
                    const clamped = Math.max(DEFAULT_MODULE_MIN_HEIGHT, Math.min(DEFAULT_MODULE_MAX_HEIGHT, nextHeight))
                    const nextHeights = {
                      ...(moduleHeights || {}),
                      [m.id]: clamped,
                    }
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {width === 2 ? (
                ...
                <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" className="opacity-90">
                  <path fill="currentColor" d="M4 5h7v14H4V5Zm9 0h7v14h-7V5ZM6 7v10h3V7H6Zm9 0v10h3V7h-3Z" />
                </svg>
              )}
            </button>
          </div>
...
function StripePaymentsModule({ paymentSettings, updatePaymentSettings, addToast }) {
    ...
    <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-3 text-xs text-slate-200">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <span className={`text-[11px] ${paymentSettings?.configured ? 'text-emerald-400' : 'text-amber-400'}`}>
          ...
          {unlocked ? (
            ...
            <button
              ...
              onClick={() => {
                setUnlocked(true)
                window.setTimeout(() => publishableKeyRef.current?.focus?.(), 0)
              }}
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          const stripePublishableKey = formData.get('stripePublishableKey')?.toString() || null
          const stripeSecretKey = formData.get('stripeSecretKey')?.toString() || null
          const stripeWebhookSecret = formData.get('stripeWebhookSecret')?.toString() || null
          const defaultCurrency = formData.get('defaultCurrency')?.toString() || null
          ...
          try {
            ...
            if (errors.length || !payload?.paymentSettings) {
              throw new Error(errors.join(', ') || 'Could not update payment settings')
            }
      ...
      >
        {!unlocked && (
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-2 text-[11px] text-slate-400">
            Click <span className="text-slate-200">Unlock</span> to edit Stripe settings.
          </div>
        )}

        <div className="space-y-1">
          <label className="text-[11px] font-medium text-slate-300">Publishable key</label>
          <input
        ...
        <div className="space-y-1">
          <label className="text-[11px] font-medium text-slate-300">Secret key</label>
          <input
        ...
        <div className="space-y-1">
          <label className="text-[11px] font-medium text-slate-300">Webhook secret (optional)</label>
          <input
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <label className={`flex items-center gap-1 text-[11px] ${unlocked ? 'text-slate-300' : 'text-slate-500'}`}>
            <input
              type="checkbox"
              name="enabled"
...
function StudioAppearanceModule({
  ...
  useEffect(() => {
    ...
    setDashboardTitle(paymentSettings.dashboardTitle || 'Pilates Studio Dashboard')
    setDefaultTheme(paymentSettings.defaultTheme === 'light' ? 'light' : 'dark')
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-3 text-xs text-slate-200">
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          try {
            const res = await updatePaymentSettings({
              variables: {
                dashboardTitle: dashboardTitle?.trim() ? dashboardTitle.trim() : null,
                defaultTheme,
              },
              refetchQueries: [PAYMENT_SETTINGS, STUDIO_SETTINGS],
            })
            ...
            const errors = payload?.errors || []
            if (errors.length || !payload?.paymentSettings) {
              throw new Error(errors.join(', ') || 'Could not update studio settings')
            }
      ...
      >
        <div className="space-y-1">
          <label className="text-[11px] font-medium text-slate-300">Dashboard title</label>
          <input
            name="dashboardTitle"
        ...
        <div className="space-y-1">
          <label className="text-[11px] font-medium text-slate-300">Default theme</label>
          <select
            name="defaultTheme"
            value={defaultTheme}
            onChange={(e) => setDefaultTheme(e.target.value === 'light' ? 'light' : 'dark')}
            className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-[11px] text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
          >
