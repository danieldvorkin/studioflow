...
export default function SignUpPortal({ accountType }) {
  ...
  const title = accountType === 'OWNER' ? 'Create owner account' : 'Create client account'
  const subtitle = accountType === 'OWNER'
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const signInRes = await signIn({ variables: { email: data.email, password: data.password } })
      ...
      try {
        if (signInPayload.user) localStorage.setItem('pilates_user', JSON.stringify(signInPayload.user))
      } catch {
        // ignore
      }

      await auth.signInWithToken(signInPayload.token, signInPayload.user || null)
      try { await client.resetStore() } catch { /* ignore */ }
      navigate('/dashboard')
    } catch (e) {
  ...
  const fillRandomTestData = async ({ submitAfter = false } = {}) => {
    ...
    try {
      ...
      setValue('name', fullName, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
      setValue('email', email, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
      if (showStudioJoin) setValue('studioInviteCode', '', { shouldDirty: true, shouldTouch: true, shouldValidate: true })
      setValue('password', nextPassword, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
      setValue('passwordConfirmation', nextPassword, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
  ...
  const handleFillRandomAndSubmit = () => fillRandomTestData({ submitAfter: true })
    ...
    <div className="min-h-screen bg-slate-950">
      <div className="grid min-h-screen md:grid-cols-2">
        ...
        <div className="flex items-center justify-center px-4 py-10 md:border-l md:border-slate-800">
          <div className={import.meta.env.DEV ? 'w-full max-w-4xl' : 'w-full max-w-md'}>
            <div className={import.meta.env.DEV ? 'grid gap-4 md:grid-cols-[1fr_340px]' : ''}>
              <div className="w-full rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-lg shadow-black/25">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-sm">
                  <div className="space-y-1">
                    ...
                    />
                    {nameRequired && (
                      <p className="text-[11px] text-slate-500">Used for bookings and receipts.</p>
                    )}
                    {showStudioJoin && (
                      <p className="text-[11px] text-slate-500">Creates a new studio workspace unless you enter a studio code.</p>
                    )}
                  </div>

                  {showStudioJoin && (
                    <div className="space-y-1">
                      <label className="block text-xs font-medium text-slate-300">Studio code (optional)</label>
                      <input
                      ...
                      />
                      <p className="text-[11px] text-slate-500">Have an existing studio? Ask the owner for the studio code.</p>
                    </div>
                  )}
                  ...
                  <div className="space-y-1">
                    ...
                    />
                    {passwordMismatch && (
                      <p className="text-xs text-rose-400">Password confirmation does not match</p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <Link to="/signup" className="hover:text-slate-200">Back</Link>
                    <Link to="/signin" className="hover:text-slate-200">Already have an account? Sign in</Link>
                  </div>
                </form>
              </div>
              ...
              {import.meta.env.DEV && (
                <div className="w-full rounded-2xl border border-slate-800 bg-slate-900/60 p-4 shadow-lg shadow-black/20">
                  <div className="rounded-xl border border-slate-800 bg-slate-950/30 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs font-medium text-slate-200">Generate test signup data</div>
                      </div>
                  ...
                  <div className="mt-3 rounded-xl border border-slate-800 bg-slate-950/30 p-3">
                    <DevSeedLoginButtons variant="bare" busy={busy} onPick={onPickDevSeed} />
                  </div>
                </div>
              )}
            </div>
...
function AuthMarketingPanel({ accountType }) {
  ...
  const heading = accountType === 'OWNER'
    ? 'Open your studio workspace.'
    : 'Book sessions with ease.'
  const copy = accountType === 'OWNER'
          ...
          >
            ...
            <g className="text-sky-400">
              ...
              <path
                ...
                strokeLinecap="round"
              />
              <circle cx="300" cy="260" r="84" fill="currentColor" opacity="0.06" />
              <circle cx="610" cy="390" r="124" fill="currentColor" opacity="0.05" />
              <circle cx="740" cy="190" r="66" fill="currentColor" opacity="0.06" />
              <path
                d="M260 470h380"
                stroke="currentColor"
                strokeOpacity="0.12"
