...
function PurchaseBundleCard({ bundleProduct, myClient, stripeConfigured, stripeAvailable, onPurchased, cardElementOptions }) {
  const { user } = useAuth()
  ...
  const onBuy = async (e) => {
    ...
    try {
      ...
      if (errors.length || !payload?.bundlePurchase) throw new Error(errors.join(', ') || 'Purchase failed')

      addToast({ message: 'Bundle purchased', type: 'success' })
      setExpanded(false)
      onPurchased?.()
    } catch (err) {
      addToast({ message: err.message || 'Could not purchase bundle', type: 'error' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-slate-50">{bundleProduct.title}</div>
          <div className="mt-0.5 text-xs text-slate-400">
            {bundleProduct.creditsCount} credits • {currencySymbol(bundleProduct.currency)}{dollarsFromCents(bundleProduct.priceCents)}
            {bundleProduct.classTemplate?.title ? ` • ${bundleProduct.classTemplate.title}` : ''}
            {bundleProduct.instructor?.name ? ` • ${bundleProduct.instructor.name}` : ''}
          </div>
          {bundleProduct.description && <div className="mt-2 text-xs text-slate-300">{bundleProduct.description}</div>}
        </div>
        ...
        <form onSubmit={onBuy} className="mt-4 space-y-3">
          ...
          {paymentChoice === 'new' && !canUseNewCard && (
            <div className="text-xs text-slate-400">
              New-card checkout is unavailable right now. Add a saved card in Profile, or try again later.
            </div>
          )}
...
export default function MyBundlesPage() {
  ...
  const {
    ...
    skip: !user || !selectedStudioId,
    variables: selectedStudioId ? { studioId: selectedStudioId } : { studioId: '0' },
    fetchPolicy: 'cache-and-network',
  })

  const shopProducts = (shopData?.bundleShopProducts || []).filter((bp) => bp?.active !== false)

  const onPurchased = async () => {
    try {
      await Promise.all([refetchPurchases?.(), refetchShop?.(), refetchMyClient?.()])
    } catch (e) {
      addToast({ message: e.message || 'Could not refresh bundles', type: 'error' })
    }
  }
  ...
  if (!selectedStudioId) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-800 bg-slate-900/80 p-6 text-sm text-slate-200">
        ...
        <p className="text-sm text-slate-400">Select a studio first to view and purchase bundles.</p>
      </div>
    )
  }
  ...
  const content = (
    <div className="w-full space-y-6">
        ...
        {!purchasesLoading && bundlePurchases.length > 0 && (
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {bundlePurchases.map((bp) => (
              <div key={bp.id} className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
                <div className="text-sm font-semibold text-slate-50">{bp.bundleProduct?.title || 'Bundle'}</div>
      ...
      <section className="space-y-3">
        ...
        {shopLoading && <p className="text-sm text-slate-400">Loading bundles…</p>}
