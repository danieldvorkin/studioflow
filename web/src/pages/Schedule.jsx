...
import { canDeleteSessions, canEditSession, canScheduleSessions, isInstructor as isInstructorUser, isOwner, isStaff } from '../auth/permissions'
...
export default function Schedule() {
  ...
  const { data: favoritesData, refetch: refetchFavorites } = useQuery(MY_FAVORITE_CLASS_SESSIONS, {
    skip: !userIsClient,
  ...
  const [toggleFavoriteSession, { loading: togglingFavorite }] = useMutation(TOGGLE_FAVORITE_CLASS_SESSION)
  ...
  const myActiveBookings = useMemo(() => {
    ...
    return raw
      .filter((b) => b && b.status !== 'cancelled' && !b.archived)
  }, [myBookingsData])

  const myBookingBySessionId = useMemo(() => {
    if (!userIsClient) return new Map()
    return new Map(myActiveBookings.map((b) => [b?.classSession?.id, b]).filter(([sid]) => !!sid))
  }, [myActiveBookings, userIsClient])

  const favoriteIds = useMemo(() => {
    if (!userIsClient) return new Set()
    return new Set((favoritesData?.myFavoriteClassSessions || []).map((s) => s.id))
  ...
  const sessionsByDay = useMemo(() => {
    return days.map((day) => {
      ...
      const filtered = sessions.filter((s) => {
        ...
        if (instructorId && (!s.instructor || s.instructor.id !== instructorId)) return false
        return true
      })
  ...
  const saveTimeEditor = async (session) => {
    ...
    const durationMins = Number.isFinite(mins) && mins > 0 ? mins : durationMinutesForSession(session)
    ...
    <div className="mx-auto flex w-full max-w-screen-2xl flex-col gap-4">
        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="grid grid-cols-7 gap-1">
            {monthWeeks.flat().map((date) => {
              ...
              const isInCurrentWeek = dayStart >= todayWeekStart && dayStart < todayWeekEnd
              ...
              return (
                <div
                  ...
                  onDrop={async (e) => {
                    ...
                    targetDayStart.setHours(start.getHours(), start.getMinutes(), 0, 0)

                    const durationMins = durationMinutesForSession(session)
                    const targetEnd = new Date(targetDayStart.getTime() + durationMins * 60000)

                    const sameDay = new Date(session.startTime).toDateString() === targetDayStart.toDateString()
                    if (sameDay) return
                    ...
                    try {
                      ...
                      addToast({ message: err.message || 'Move failed', type: 'error' })
                    }
                  }}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {daySessions.length > 0 && (
                      <span className="text-[11px] text-slate-400">{daySessions.length}</span>
                    )}
                  </div>

                  <div className="mt-1 flex flex-col gap-1 overflow-auto pr-1" style={{ maxHeight: 104 }}>
                    {daySessions.map((s) => (
                      <div
                        key={s.id}
                        ...
                        onMouseMove={(e) => hoveredSession?.id === s.id && showTooltip(s, e)}
                      ...
                      >
                        <div className="min-w-0 flex-1 truncate">
                          {formatTime(s.startTime)} {s.classTemplate?.title || 'Class'}
                        </div>
                            ...
                            <button
                              ...
                              onClick={async (e) => {
                                ...
                                try {
                                  ...
                                  if (end && Number.isNaN(end.getTime())) throw new Error('Invalid session end time')

                                  let createdSession = null
                                  let usedWeeks = null
                                  let lastError = null

                                  for (let weeksAhead = 1; weeksAhead <= 8; weeksAhead += 1) {
                                    const candidateStart = new Date(start)
                                    candidateStart.setDate(candidateStart.getDate() + 7 * weeksAhead)

                                    const candidateEnd = end
                                      ? new Date(end.getTime() + 7 * weeksAhead * 24 * 60 * 60 * 1000)
                                      : null
                                    ...
                                    lastError = errors.join(', ') || 'Could not duplicate session'
                                    const canRetry = errors.some((msg) => /start time|overlap|taken/i.test(msg))
                                    if (!canRetry) break
                                  }

                                  if (!createdSession) throw new Error(lastError || 'Could not duplicate session')

                                  addToast({
                                    message: usedWeeks === 1 ? 'Session duplicated (next week)' : `Session duplicated (+${usedWeeks} weeks)`,
                                    type: 'success',
                                  })
                                  await refetch()
                                } catch (err) {
                                  addToast({ message: err.message || 'Duplicate failed', type: 'error' })
                                }
                              }}
                          ...
                          {canDeleteSessions(currentUser) && (
                            <button
                              ...
                              onClick={async (e) => {
                                ...
                                try {
                                  const res = await deleteClassSession({ variables: { id: s.id } })
                                  const payload = res.data?.deleteClassSession
                                  if (!payload?.success) {
                                    throw new Error((payload?.errors || ['Cancel failed']).join(', '))
                                  }
                                  addToast({ message: 'Session cancelled', type: 'success' })
                                  await refetch()
                                } catch (err) {
                                  addToast({ message: err.message || 'Cancel failed', type: 'error' })
                                }
                              }}
          ...
          <div className="mt-3 rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm shadow-black/20">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Events</div>
                <div className="mt-1 text-sm font-semibold text-slate-50">
                  {selectedMonthDate ? formatDay(selectedMonthDate) : 'Select a day'}
                </div>
              </div>
              {selectedMonthDate && (
                <div className="text-xs text-slate-500">{selectedMonthSessions.length} sessions</div>
              )}
            </div>

            {!selectedMonthDate ? (
              <p className="mt-3 text-sm text-slate-400">Click a day above to see its schedule.</p>
            ) : selectedMonthSessions.length === 0 ? (
              <p className="mt-3 text-sm text-slate-400">No classes scheduled for this day.</p>
            ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 text-xs text-slate-400">
                          {formatTimeRange(
                            s.startTime,
                            s.endTime || new Date(new Date(s.startTime).getTime() + durationMinutesForSession(s) * 60000).toISOString(),
                          )}
                        </div>
                      </div>
                      <div className="shrink-0 text-[11px] text-slate-500">{formatTime(s.startTime)}</div>
                    </div>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                        {userIsClient && (
                          <button
                            ...
                            onClick={async () => {
                              try {
                                const res = await toggleFavoriteSession({ variables: { classSessionId: s.id } })
                                const payload = res.data?.toggleFavoriteClassSession
                                const errors = payload?.errors || []
                                if (errors.length) throw new Error(errors.join(', '))
                                await refetchFavorites()
                                addToast({ message: payload?.favorited ? 'Saved to favorites' : 'Removed from favorites', type: 'success' })
                              } catch (e) {
                                addToast({ message: e.message || 'Could not update favorite', type: 'error' })
                              }
                            }}
                            className={favoriteIds.has(s.id)
                            ...
                            aria-label={favoriteIds.has(s.id) ? 'Unfavorite session' : 'Favorite session'}
                        ...
                        {userIsClient && (() => {
                          ...
                          const isFull = typeof s.seatsAvailable === 'number' && s.seatsAvailable <= 0
                          if (isFull) {
                            return (
                        ...
                        {canCreateSessions && (
                          <>
                            ...
                            <button
                              type="button"
                              onClick={() => setMonthExpandedSessionId((prev) => (prev === s.id ? null : s.id))}
                      ...
                      <div className="mt-2 rounded-xl border border-slate-800 bg-slate-950/40 p-2">
                        {(() => {
                          ...
                          if (sessionBookings.length === 0) {
                            return <div className="text-xs text-slate-400">No active bookings for this session.</div>
                          }
          ...
          {sessionsByDay.map(({ date, sessions: daySessions }) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-500">{daySessions.length} classes</span>
              </div>
                      ...
                      {editingSessionId === s.id ? (
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="text-[11px] text-slate-400">
                              {formatTimeRange(s.startTime, s.endTime || new Date(new Date(s.startTime).getTime() + durationMinutesForSession(s) * 60000).toISOString())}
                            </div>
                          </div>
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                            <div className="space-y-1">
                              <label className="text-[11px] font-medium text-slate-300">Start</label>
                              <input
                                type="datetime-local"
                            ...
                            <div className="space-y-1">
                              <label className="text-[11px] font-medium text-slate-300">Duration (min)</label>
                              <input
                                type="number"
                                min="1"
                                className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                        ...
                        <>
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="text-[11px] text-slate-400">
                              {s.endTime ? formatTimeRange(s.startTime, s.endTime) : formatTime(s.startTime)}
                            </div>
                          </div>
                          ...
                          {(canEditSession(currentUser, s) || canDeleteSessions(currentUser)) && (
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                              {canEditSession(currentUser, s) && (
                                <>
                                  <button
                                  ...
                                  <button
                                    ...
                                    onClick={async () => {
                                      ...
                                      try {
                                        const res = await updateClassSession({ variables: { id: s.id, room: newRoom || null } })
                                        const errors = res.data?.updateClassSession?.errors || []
                                        if (errors.length) throw new Error(errors.join(', '))
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const res = await deleteClassSession({ variables: { id: s.id } })
                                      const payload = res.data?.deleteClassSession
                                      if (!payload?.success) throw new Error((payload?.errors || ['Cancel failed']).join(', '))
                                      addToast({ message: 'Session cancelled', type: 'success' })
                                      await refetch()
                                    } catch (e) {
      ...
      {showNewEventModal && viewMode === 'month' && (
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">New event</div>
                <div className="mt-1 text-sm font-semibold text-slate-50">Schedule a session</div>
              </div>
              <button
                type="button"
                onClick={() => closeNewEventModal()}
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setNewEventForm((f) => ({ ...f, classTemplateId: e.target.value }))}
                >
                  {templates.map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.title}
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-300">Capacity (optional)</label>
                  <input
                    type="number"
                    min="1"
                    className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                    value={newEventForm.capacity}
                    onChange={(e) => setNewEventForm((f) => ({ ...f, capacity: e.target.value }))}
                  />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-300">Room (optional)</label>
                <input
                  type="text"
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                  value={newEventForm.room}
                  onChange={(e) => setNewEventForm((f) => ({ ...f, room: e.target.value }))}
                />
              </div>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (!newEventForm.classTemplateId) throw new Error('Class template is required')
                      if (!newEventForm.time) throw new Error('Time is required')
      ...
      {viewMode === 'month' && hoveredSession && (
        <div
          ...
          style={{ left: tooltipPos.x + 12, top: tooltipPos.y + 12, pointerEvents: 'none' }}
        >
          ...
          <div className="mt-0.5 text-[11px] text-slate-300">
            {formatTimeRange(
              ...
              hoveredSession.endTime || new Date(new Date(hoveredSession.startTime).getTime() + durationMinutesForSession(hoveredSession) * 60000).toISOString(),
            )}
            {' · '}
          ...
          <div className="mt-2 space-y-1 text-[11px] text-slate-200">
            {hoveredSession.instructor?.name && (
              <div className="flex justify-between gap-3">
                ...
                <span className="truncate">{hoveredSession.instructor.name}</span>
              </div>
            )}
            {hoveredSession.room && (
              <div className="flex justify-between gap-3">
