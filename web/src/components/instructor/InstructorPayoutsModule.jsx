...
function StatusPill({ tone, children }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.2em] ${classes[tone] || classes.slate}`}
    >
      {children}
    </span>
  );
...
function EarningsCard({
  ...
  const payoutTone =
    payout?.status === "paid" ? "emerald" : payout ? "amber" : "slate";
    ...
    <article className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <div className="truncate text-sm font-semibold text-slate-50">
              {instructor?.name || instructor?.email || "Instructor"}
            </div>
            <StatusPill tone="sky">
              {(row.currency || "cad").toString().toUpperCase()}
            </StatusPill>
            <StatusPill tone={payoutTone}>
              {payout ? payout.status : "no payout"}
            </StatusPill>
          </div>
          {instructor?.email && (
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950/20 p-2">
          ...
          <div className="mt-0.5 text-sm font-semibold text-slate-50">
            {formatMoney(row.instructorEarningsCents, row.currency)}
          </div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-950/20 p-2">
          <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500">
          ...
          <div className="mt-0.5 text-sm font-semibold text-slate-200">
            {formatMoney(row.studioCutCents, row.currency)}
          </div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-950/20 p-2">
          <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500">
      ...
      {showDetails && (
        <div className="mt-4 space-y-3">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <section className="rounded-xl border border-slate-800 bg-slate-950/20 p-3">
              <div className="mb-2 flex items-center justify-between">
                <h4 className="text-xs font-semibold uppercase tracking-[0.25em] text-sky-400">
                  Payout
                </h4>
                {payout?.paidAt && (
              ...
              {!payout ? (
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      addToast({
                        message: err.message || "Could not create payout",
                        type: "error",
                      });
                    }
                  }}
                ...
                <div className="space-y-2">
                  <div className="text-xs text-slate-300">
                    Status:{" "}
                    <span className="text-slate-100">{payout.status}</span>
                    {payout.paidMethod && (
                      <>
                        {" · "}via{" "}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      type="button"
                      disabled={
                        !(connectId && connectOk) || payout.status === "paid"
                      }
                      title={
                        connectId
                          ? connectOk
                            ? "Pay via Stripe transfer"
                            : "Stripe account not ready (onboarding incomplete)"
                      ...
                      onClick={async () => {
                        try {
                          const res = await payPayout({
                            variables: { id: payout.id },
                          });
                    ...
                    <form
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (!method)
                            throw new Error("Paid method is required");
            ...
            <section className="rounded-xl border border-slate-800 bg-slate-950/20 p-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {connectId ? (
                    <StatusPill tone={connectOk ? "emerald" : "amber"}>
                      {connectOk ? "Connected" : "Incomplete"}
                    </StatusPill>
                  ) : (
              ...
              <div className="space-y-2 text-xs text-slate-300">
                      ...
                      onClick={async () => {
                        try {
                          const res = await refreshConnectStatus({
                            variables: { instructorId: instructor.id },
                          });
                          const payload =
                            res.data?.refreshInstructorConnectStatus;
                          const errors = payload?.errors || [];
                          if (errors.length) throw new Error(errors.join(", "));
                          addToast({
                ...
                {!connectOk && (
                  <div className="text-[11px] text-slate-500">
                    Generate an onboarding link, complete it in Stripe, then
                    refresh status.
                  </div>
                )}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const payload =
                          res.data?.createInstructorConnectOnboarding;
                        const errors = payload?.errors || [];
                        if (errors.length) throw new Error(errors.join(", "));
                        ...
                        addToast({
                          message:
                            err.message || "Could not create onboarding link",
                          type: "error",
                        });
                      }
                    }}
          ...
          {Array.isArray(row.templateBreakdown) &&
            row.templateBreakdown.length > 0 && (
              <section className="rounded-xl border border-slate-800 bg-slate-950/20 p-3">
                <div className="mb-2 text-xs font-semibold uppercase tracking-[0.25em] text-sky-400">
                  Breakdown
                </div>
                  ...
                  <table className="min-w-full text-left text-xs text-slate-300">
                    ...
                    <tbody>
                      {row.templateBreakdown.map((b) => (
                        <tr
                          key={b.classTemplate?.id || b.classTemplate?.title}
                          className="border-t border-slate-800"
                        >
                          <td className="px-3 py-2 text-slate-50">
                            {b.classTemplate?.title || "—"}
                          </td>
                          <td className="px-3 py-2 text-right text-slate-200">
                            {formatMoney(b.grossCents, row.currency)}
                          </td>
                          <td className="px-3 py-2 text-right text-slate-50">
                            {formatMoney(
                              b.instructorEarningsCents,
                              row.currency,
                            )}
                          </td>
                        </tr>
...
export default function InstructorPayoutsModule({ addToast, studioId }) {
  ...
  const instructors = useMemo(() => {
    const list = users || [];
    return list.filter(
      (u) => (u.roleName || "").toLowerCase() === "instructor" || u.role === 2,
    );
  }, [users]);
  ...
  } = useQuery(INSTRUCTOR_EARNINGS_WEEKS, {
    variables: {
  ...
  const [createConnectOnboarding] = useMutation(
    CREATE_INSTRUCTOR_CONNECT_ONBOARDING,
  );
    ...
    <div className="flex h-full min-h-0 flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
      ...
      <details className="mt-4 rounded-xl border border-slate-800 bg-slate-900/60 p-3">
        <summary className="cursor-pointer select-none text-xs font-semibold uppercase tracking-[0.25em] text-sky-400">
          ...
          <span className="ml-2 text-[10px] normal-case tracking-normal text-slate-500">
            (instructor defaults + template overrides)
          </span>
        </summary>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500">
                applies when template overrides are blank
              </span>
            </div>

            <div className="space-y-2">
              {instructors.length === 0 && (
                <div className="text-sm text-slate-500">No instructors.</div>
              )}
              ...
              {instructors.map((i) => (
                <form
                  ...
                  onSubmit={async (e) => {
                    ...
                    const instructorCompensationType =
                      fd.get("type")?.toString() || null;
                    const instructorDefaultSplitPercent = Number(
                      fd.get("splitPercent") || 0,
                    );
                    const instructorDefaultFlatRateCents = Number(
                      fd.get("flatRateCents") || 0,
                    );
                    const stripeConnectAccountId =
                      (
                        fd.get("stripeConnectAccountId")?.toString() || ""
                      ).trim() || null;
                    ...
                    try {
                      ...
                      if (errors.length) throw new Error(errors.join(", "));
                      addToast({
                        message: "Instructor defaults saved",
                        type: "success",
                      });
                      refetchEarnings();
                    } catch (err) {
                ...
                >
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                      <div className="truncate text-[11px] text-slate-50">
                        {i.name || i.email}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[11px]">
                      <select
                        name="type"
                        defaultValue={
                          i.instructorCompensationType || "revenue_share"
                        }
                        className="rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-[11px] text-slate-100 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                      >
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
            ...
            {!templatesLoading && classTemplates.length === 0 && (
              <div className="text-sm text-slate-500">No templates.</div>
            )}
                  ...
                  <form
                    ...
                    onSubmit={async (e) => {
                      ...
                      const compensationType =
                        fd.get("compensationType")?.toString() || null;
                      const splitRaw = fd.get("splitPercent")?.toString();
                      const flatRaw = fd.get("flatRateCents")?.toString();
                      const instructorSplitPercent =
                        splitRaw === "" || splitRaw == null
                          ? null
                          : Number(splitRaw);
                      const instructorFlatRateCents =
                        flatRaw === "" || flatRaw == null
                          ? null
                          : Number(flatRaw);
