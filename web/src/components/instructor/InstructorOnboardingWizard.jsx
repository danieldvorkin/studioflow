...
function StepDot({ active, done, label, idx }) {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span
        className={`text-[10px] uppercase tracking-[0.18em] ${active ? "text-sky-400" : done ? "text-sky-500/70" : "text-slate-600"}`}
      >
        {label}
      </span>
    </div>
...
function StepConnector({ done }) {
  return (
    <div
      className={`mb-4 h-px flex-1 transition-colors ${done ? "bg-sky-500/50" : "bg-slate-800"}`}
    />
  );
}
...
function FieldLabel({ children, optional }) {
  return (
    <div className="mb-1 flex items-center gap-2">
      <label className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-400/70">
        {children}
      </label>
      {optional && (
        <span className="rounded-full bg-slate-800 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-slate-500">
          optional
        </span>
      )}
    </div>
...
const INPUT =
  "w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none placeholder:text-slate-500 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition";
...
export default function InstructorOnboardingWizard({ open, onClose, onDone }) {
  ...
  const [createConnect, { loading: stripeLoading }] = useMutation(
  ...
  async function handleCompNext() {
    ...
    const vars = {
      ...
      instructorDefaultSplitPercent:
        compType === "split" ? Number(splitPct) : null,
      instructorDefaultFlatRateCents:
        compType === "flat" ? Math.round(Number(flatCents || 0) * 100) : null,
    };
  ...
  async function handleSendStripe() {
    ...
    try {
      const res = await createConnect({
        variables: { instructorId: createdUserId },
      });
        ...
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <div>
            ...
            <div className="mt-0.5 text-xs text-slate-500">
              Set up a new instructor account for your studio
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex items-center gap-0 px-6 pt-5">
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex flex-1 items-center">
              ...
              />
              {i < STEPS.length - 1 && <StepConnector done={stepIdx > i} />}
            </div>
          ))}
        </div>
        ...
        <div className="px-6 pb-6 pt-4">
          ...
          {step === 0 && (
            <div className="flex flex-col gap-4">
              <p className="text-sm text-slate-400">
                Enter the instructor's details. They'll receive an email to set
                their password and join your studio.
              </p>
          ...
          {step === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <p className="text-sm text-slate-400">
                Set this instructor's default pay structure. This can be
                overridden per class template later.
              </p>
              ...
              {compType === "split" && (
                <Field label="Instructor split %">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <span className="w-12 text-right text-sm font-semibold text-sky-400">
                      {splitPct}%
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
                    Instructor earns{" "}
                    <strong className="text-slate-300">{splitPct}%</strong> of
                    each class's gross revenue. Studio keeps{" "}
              ...
              {compType === "flat" && (
                <Field label="Flat rate per class">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Instructor earns a fixed rate for each class they teach,
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
                ...
                <p className="text-sm text-slate-400">
                  Connect this instructor to Stripe so you can pay them directly
                  from within StudioFlow. They'll complete a short Stripe
                  onboarding flow on their own device.
                </p>
              </div>
              ...
              {stripeStatus === "sent" && stripeUrl && (
                <div className="flex flex-col gap-3">
                  <div className="rounded-xl border border-sky-500/20 bg-sky-950/20 px-4 py-3">
                    <div className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-sky-400/70">
                      Onboarding link
                    </div>
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    They'll complete Stripe's form on their own. You can check
                    their status later from Instructor payouts.
                  </p>
                </div>
              )}
              ...
              {stripeStatus === "error" && (
                <p className="text-xs text-slate-500">
                  You can set up Stripe Connect later from the Instructor
                  payouts page.
                </p>
              )}
...
function SummaryRow({ icon, label, value, muted }) {
  return (
    <li
      className={`flex items-start gap-3 rounded-xl border px-4 py-3 ${muted ? "border-slate-800 bg-slate-900/40" : "border-emerald-500/20 bg-emerald-950/15"}`}
    >
      <span className="mt-0.5 text-base leading-none">{icon}</span>
      <div className="min-w-0">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-400/70">
          {label}
        </div>
        <div
          className={`mt-0.5 text-sm ${muted ? "text-slate-500" : "text-slate-200"}`}
        >
          {value}
        </div>
      </div>
