    ...
    (id) => {
      ...
      setToasts((prev) =>
        prev.map((t) => {
          ...
          if (createdAt && maxLifetimeMs > 0) {
            hardRemainingMs = Math.max(
              createdAt + maxLifetimeMs - Date.now(),
              0,
            );
          }
      ...
      const delayMs =
        hardRemainingMs == null
          ? remainingToSchedule
          : Math.min(remainingToSchedule, hardRemainingMs);
      if (delayMs <= 0) {
        removeToast(id);
        return;
      }
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex flex-col items-end gap-2 p-4 sm:items-center">
        {toasts.map((toast) => (
          <div
            ...
            className={`pointer-events-auto w-full max-w-sm rounded-lg border px-3 py-2 text-sm shadow-lg backdrop-blur ${
              toast.type === "error"
