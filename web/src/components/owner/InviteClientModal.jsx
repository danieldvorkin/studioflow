...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${map[status] ?? map.expired}`}
    >
      {status}
    </span>
  );
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Send a personalised signup link to a new client.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-6">
          ...
          {invitations.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-sky-400">
                Sent invitations
              </h3>
                ...
                {invitations.map((inv) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 ml-3 shrink-0">
                      ...
                      {inv.status === "pending" && (
                        <CopyButton
                          value={`${window.location.origin}/signup/client?token=${inv.token}`}
                        />
                      )}
                    </div>
                  </li>
